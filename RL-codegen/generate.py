...
def main(args):
    ...
    elif args.reward_scores:
        model = T5ForConditionalGeneration.from_pretrained(args.model_path, tuning_mode='reward-model')
    else:
    ...
    for index, problem in tqdm(enumerate(problems), ncols=0, total=len(problems)):
        ...
        if (args.critic_scores or args.reward_scores) and not args.gt_solutions:
        ...
        if args.critic_scores or args.reward_scores:
        ...
        with torch.no_grad():
            if args.critic_scores:
                ...
                # Make batches
                error_pred_list, error_hiddens = [], []
                for i in range(text_tensor.shape[0]):
                    curr_inputs = {'input_ids': text_tensor[i].unsqueeze(0), 'error_types': gt_error_tensor[i].unsqueeze(0), 'labels': code_tensor[i].unsqueeze(0)}
                    _, error_preds, error_hidden_states = model(**curr_inputs, return_error_hidden_states=True)
                    error_pred_list.append(error_preds)
                    error_hiddens.append(error_hidden_states)
                error_preds = torch.cat(error_pred_list, dim=0)
                error_hidden_states = torch.cat(error_hiddens, dim=0)
                ...
                # Make batches
                reward_list, reward_hiddens_list = [], []
                for i in range(text_tensor.shape[0]):
                    curr_inputs = {'input_ids': text_tensor[i].unsqueeze(0),
                                   'error_types': gt_error_tensor[i].unsqueeze(0),
                                   'labels': code_tensor[i].unsqueeze(0)}
                    reward, reward_hiddens = model(**curr_inputs, return_error_hidden_states=True)
                    reward_list.append(reward)
                    reward_hiddens_list.append(reward_hiddens)
                reward_preds = torch.Tensor(reward_list)
                reward_hidden_states = torch.cat(reward_hiddens_list, dim=0)

                assert len(gt_error_types) == len(reward_preds)
                # all_preds.extend(reward_preds.cpu().numpy().tolist())
                # all_gts.extend(gt_error_types)

                saved_reward_scores = {'code': input_codes, 'prompt': input_texts,
                                       'gt_error_type': gt_error_types,
                                       'pred_error_type': reward_preds.cpu().numpy(),
                                       'error_hidden_states': reward_hidden_states.cpu().numpy()}

                if args.gt_solutions:
                    scores_loc = os.path.join(prob_path, "solutions_reward_scores{}.pkl".format(args.score_name))
                else:
                    scores_loc = os.path.join(prob_path, "gen_solutions_reward_scores{}.pkl".format(args.score_name))

                pkl.dump(saved_reward_scores, open(scores_loc, 'wb'))
                ...
                saved_codes = {}
                if args.logits:
                    saved_codes[problem_id] = {'code': output_programs, 'prompt': input_text, 'scores': scores}
                else:
