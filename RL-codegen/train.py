...
from datasets.pairwise_dataset import APPSPairwiseDataset
...
def run_training(args, train_data):
    ...
    if args.tuning_mode in ['critic', 'rl', 'reward-model']:
        trainer = Trainer_RL(
            model=model,
            args=training_args,
...
def get_dataset(args): 
    ...
    if args.model in ['codet5-base', 'codet5-large']:
        ...
        max_tokens = 1024
        max_src_tokens = -1
    print("init returns", args.returns)
    returns = [float(x) for x in args.returns.split("_")[1:]]
    print(returns)
    if args.tuning_mode not in ["reward-model"]:
        ...
        train_data = APPSPairwiseDataset(
            dataroot=args.train_path,
            problem_dirs=fnames,
            model=args.model,
            max_tokens=max_tokens,
