...
class APPSBaseDataset(torch.utils.data.Dataset):
    ...
    def load_gen_samples(self, sols, answer_type, starter_code, question_str, f_name):
        samples = []
        info = []
        
        for idx, sol in enumerate(sols):
            sol_str = sol['code']  
            sample = (question_str, starter_code, sol_str, answer_type, f_name)
    ...
    def load_rl_samples(self, sols, baseline_error_type, f_name):
        samples = []
        info = []
        
        for idx, code in enumerate(sols['code']):   
            samples.append((sols['prompt'][idx], code, f_name))
            info.append((sols['gt_error_type'][idx], sols['error_hidden_states'][idx], baseline_error_type))
            
        return samples, info 
     
    def load_gt_samples(self, sols, answer_type, starter_code, question_str, f_name):
        samples = []
    ...
    def initialize(self):
        ...
        for problem_name in tqdm(self.problem_dirs):           
            ...
            gt_samples = self.load_gt_samples(sols_str_list, answer_type, starter_code, question_str, problem_name)
            ...
            if self.tuning_mode in ['critic']: 
                for fname in gen_sols_fname:
                    if os.path.exists(fname):
                        ...
                        samples, info = self.load_gen_samples(gen_sols, answer_type, starter_code, question_str, problem_name)
                ...
                for fname in gen_sols_fname: 
                    if os.path.exists(fname):
                        ...
                        samples, info = self.load_rl_samples(gen_sols, baseline_error_type, problem_name)
        ...
        if self.relative_returns: 
            ...
            sample_rewards = np.array([dsutils.get_reward_from_error_type(e, self.returns) for e in self.all_error_types])
            baseline_rewards = np.array([dsutils.get_reward_from_error_type(e, self.returns) for e in self.all_baseline_error_types])
    ...
    def pack_samples(self, idx, sample_type=None):
        ...
        if self.sample_mode == 'uniform_sol':
            # (question_str, starter_code, sol_str, answer_type, f_name)
            curr_q, curr_s, curr_a, curr_q_prefix, f_name = sample_pool[idx]
        ...
        while curr_num_tokens < self.max_tokens:
            ...
            if self.tuning_mode in ['critic'] and sample_type=='gen': 
                curr_samples.append((curr_q, curr_s, curr_a, curr_q_prefix, f_name,
            ...
            if self.sample_mode == 'uniform_sol':
                ...
                curr_q, curr_s, curr_a, curr_q_prefix, f_name = sample_pool[new_idx]
            elif self.sample_mode == 'uniform_prob':
                raise NotImplementedError()
    ...
    def sample_task(self, samples, sample_type=None):
        ...
        for sample in samples:
            if self.tuning_mode in ['critic'] and sample_type == 'gen': 
                q_str, s_str, a_str, answer_type, f_name, result, error_subtype = sample
            else:
                q_str, s_str, a_str, answer_type, f_name = sample
    ...
    def sample_rl_task(self, item, info):
        ...
        if self.relative_returns: 
            curr_reward = dsutils.get_reward_from_error_type(gt_error, self.returns)
            baseline_reward = dsutils.get_reward_from_error_type(baseline_error, self.returns) if baseline_error!=-1 else 0
            relative_reward = curr_reward - baseline_reward
            rewards *= relative_reward
        else:
            rewards *= dsutils.get_reward_from_error_type(gt_error, self.returns)
