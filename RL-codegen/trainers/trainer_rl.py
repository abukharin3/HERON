...
class Trainer_RL:
    ...
    def __init__(
        ...
        self.eval=eval
        self.critic_scores_path=critic_scores_path
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]], step) -> torch.Tensor:
        ...
        with self.autocast_smart_context_manager():
            ...
            elif self.tuning_mode in ['reward-model']:
                loss, acc = self.compute_loss(model, inputs, step)
    ...
    def compute_loss(self, model, inputs, step, return_outputs=False):
        ...
        if self.tuning_mode in ['critic']: 
            ...
            outputs = None
            curr_inputs = {'input_ids': inputs['input_ids'], 'labels1': inputs['labels1'], 'labels2': inputs['labels2']}
            reward_loss, acc = model(**curr_inputs)
