...
if __name__ == "__main__":
	...
	model = HERON("MlpPolicy", env, verbose=True, learning_rate=3e-4, heirarchy=order, sigma=sigmas, factor_dim=factor_dim, multiple_sigmas=multiple_sigmas, heron=False)
	...
	np.save(f"results/PPO_{args.env}_{args.time_steps}_{args.seed}.npy", np.array(results.stored_rewards))
	...
	stats_path = os.path.join(f"results/ppo_{args.env}_{args.time_steps}_{args.seed}.pkl")#, "vec_normalize.pkl")
	env.save(stats_path)
